...
export default function HRPage() {
  ...
  const { data: attendance, isLoading: loadingAttendance } = useAttendance(today);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <TableBody>
                      {employees?.map((emp) => (
                        <TableRow key={emp.id} className="border-slate-800 hover:bg-slate-800/40">
                          ...
                          <TableCell>
                            <Badge className={
                              emp.attendance_status === 'Present' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                              emp.attendance_status === 'Late' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
        ...
        <TabsContent value="attendance" className="mt-6">
          <Card className="bg-slate-900 border-slate-800">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-white">Attendance Logs - {today}</CardTitle>
                <CardDescription>Manual check-ins and lock status indicator.</CardDescription>
              </div>
              <Button 
                onClick={() => lockAttendance.mutate(today)}
                          ...
                          <TableCell>
                            {row.is_locked ? (
                              ...
                              <div className="flex items-center text-slate-500 text-sm">
                                <Unlock size={14} className="mr-1" /> Editable
                              </div>
                            )}
                          </TableCell>
                          <TableCell className="text-right">
                             <Button variant="ghost" size="sm" disabled={row.is_locked}>Edit</Button>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
